      ...
      <div class={classJoin('level', 'section-content', style.organizersContainer)}>
        {members.map((member) =>
          (<div className={classJoin('level-item', 'has-text-centered', style.organizer)}>
            <figure>
              <a aria-label={`Perfil de Twitter de ${member.twitter}`} href={`https://twitter.com/${member.twitter}`}><img src={`../../assets/images/organizers/${member.image}`} alt={`Perfil de Twitter de ${member.twitter}`} className={style.memberImage} /></a>
